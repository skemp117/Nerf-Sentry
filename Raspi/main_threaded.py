...
def get_aim_center(foes):
    return np.transpose(
        np.array([np.mean(foes[:,[1, 3]], axis = 1), 
                np.average(foes[:,[0, 2]], axis = 1, weights = (3,1))])) # (+right, +down)

def select_foe(foes, last_target_id):
    if foes.shape[0] == 0:
    ...
    if foe.size > 0:
        ...
        center_pts = get_aim_center(foes) 
...
def get_range(pixel_height):
    return REFERENCE_HEIGHT * FOCAL_LENGTH / (pixel_height * (SENSOR_VERTICAL_SIZE / SRC_SIZE[1])) * METER_TO_MM
...
def process_inference():
    ...
    while True:
        ...
        if detections.size == 0:
            ...
            foe, target_params, last_target_id = select_foe(foes, last_target_id)
            fire_signal = target_list.add_target(foe)
            # print(target_params)
...
if __name__ == "__main__":
    process_inference_queue = queue.Queue(maxsize=1)

    thread1 = threading.Thread(target = get_image_and_infer)
    thread2 = threading.Thread(target = process_inference)
